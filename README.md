...
| 流式输出 | 回答逐字返回，首字约 2-3 秒；决策路径与工具调用在同一轮里同步刷新，不用等整段生成完 | `EVAdvisor.stream_chat()` 生成器（langgraph `stream_mode=["updates","messages"]`）+ Gradio 流式 yield |
...
回答本身是**流式输出**的：先亮出意图与执行路径，工具调用一发生就显示，
最后答案逐字出现，不用盯着转圈等十几秒。
